...
public:
    ...
    // Методы для админа
    bool addUser(const QList<QString> params); // метод добавления пользователя
    void deleteUser(const QString username); // метод поиска пользователя
    void findAllIncome(); // метод нахождения общего дохода
    bool addPlastic(const QString &name, const QString &cost); // метод добавления пластика
    void delPlastic(const QString &name); // метод удаления пластика
