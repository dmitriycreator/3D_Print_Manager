...
double Database::findIncome(const QString &orderid)
{
    ...
    addIncome(income); // делаем запись в таблице доходов
...
double Database::findConsumtion(const QString &orderid)
{
    ...
    addIncome(consumption); // делаем запись в таблице доходов

    return consumption;
}

// Метод добавления дохода в таблицу
void Database::addIncome(const double &income)
{
    if(income == 0)
        return;
    ...
    query->prepare(QString("INSERT INTO income( date, income) "
                           "VALUES(:date, :income)"));
