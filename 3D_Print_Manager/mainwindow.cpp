...
void MainWindow::on_action_createDB_triggered()
{
    ...
    role = "admin"; // роль админ так как база впервые создана
...
void MainWindow::on_action_openDB_triggered()
{
    ...
    role = "manager"; // роль менеджер для стандартного открытия БД
    this->hide(); // вызываем авторизацию
    ui_Auth.show();
}
...
{
    if(m_loginSuccesfull) // если пользователь уже был зарегистрирован, значит в layout есть старое окно
    {
        ui->gridLayout->itemAt(0)->widget()->hide(); // прячем и убираем его из layout
        ui->gridLayout->removeItem(ui->gridLayout->itemAt(0));

        /* При удалении виджета программа ломается и новую БД уже не открыть
         * ui->gridLayout->removeItem(ui->gridLayout->itemAt(0)->widget()); */

        ui->gridLayout->update();
    }
    // В зависисмости от роли пользователя создаем новое окно
    if(role == "manager")
    {
        manager = new ManagerTabWidget;
    ...
    this->show();
    m_loginSuccesfull = true; // флаг успешной авторизации
}
